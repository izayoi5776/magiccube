...
public class DemoApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		int x = 0;
		// 27:ESC
		while(x!=Config.KEY_ESC) {
			x = RawConsoleInput.read(true);
			//x = Config.KEY_LEFT;
			System.out.println("input=" + Config.key2Str(x));
			//                Ctrl
			// left 57419     57459
			// right 57421    57460
			// up 57416       57485
	...
	public String atos() {
		String s = "";
		// side0
		s += "   " + vl(0) + a[0][0][0] + vl(1) + a[0][0][1] + vl(2) + a[0][0][2] + vl(3) + "\n";
		s += "   " + vl(0) + a[0][1][0] + vl(1) + a[0][1][1] + vl(2) + a[0][1][2] + vl(3) + "\n";
		s += "   " + vl(0) + a[0][2][0] + vl(1) + a[0][2][1] + vl(2) + a[0][2][2] + vl(3) + "\n";

		// side 1236
		s += hori==0 ? "---------------\n" : "";
		s += "" + a[1][0][0] + a[1][0][1] + a[1][0][2];
		s += "" + vl(0) + a[2][0][0] + vl(1) + a[2][0][1] + vl(2) + a[2][0][2] + vl(3);
		s += "" + a[3][0][0] + a[3][0][1] + a[3][0][2];
		s += "" + a[6][0][0] + a[6][0][1] + a[6][0][2] + "\n";

		s += hori==0 || hori==1 ? "---------------\n" : "";
		s += "" + a[1][1][0] + a[1][1][1] + a[1][1][2];
		s += "" + vl(0) + a[2][1][0] + vl(1) + a[2][1][1] + vl(2) + a[2][1][2] + vl(3);
		s += "" + a[3][1][0] + a[3][1][1] + a[3][1][2];
		s += "" + a[6][1][0] + a[6][1][1] + a[6][1][2] + "\n";

		s += hori==1 || hori==2 ? "---------------\n" : "";
		s += "" + a[1][2][0] + a[1][2][1] + a[1][2][2];
		s += "" + vl(0) + a[2][2][0] + vl(1) + a[2][2][1] + vl(2) + a[2][2][2] + vl(3);
		s += "" + a[3][2][0] + a[3][2][1] + a[3][2][2];
		s += "" + a[6][2][0] + a[6][2][1] + a[6][2][2] + "\n";
		s += hori==2 ? "---------------\n" : "";

		// side 4
		s += "   " + vl(0) + a[4][0][0] + vl(1) + a[4][0][1] + vl(2) + a[4][0][2] + vl(3) + "\n";
		s += "   " + vl(0) + a[4][1][0] + vl(1) + a[4][1][1] + vl(2) + a[4][1][2] + vl(3) + "\n";
		s += "   " + vl(0) + a[4][2][0] + vl(1) + a[4][2][1] + vl(2) + a[4][2][2] + vl(3) + "\n";
		
		// side 5
		s += "   ----\n";
		s += "   " + vl(0) + a[5][0][0] + vl(1) + a[5][0][1] + vl(2) + a[5][0][2] + vl(3) + "\n";
		s += "   " + vl(0) + a[5][1][0] + vl(1) + a[5][1][1] + vl(2) + a[5][1][2] + vl(3) + "\n";
		s += "   " + vl(0) + a[5][2][0] + vl(1) + a[5][2][1] + vl(2) + a[5][2][2] + vl(3) + "\n";

		return s;
	}
	/**
	 * 返回垂直线
	 * @param pos	0:最左，1：1个右侧，2：2个右侧，3：最右
	 * @return 需要显示的垂直线或者空文字列
	 */
	private String vl(int pos) {
		String BK = "";
		final String PP = "|";
		switch(vert) {
		case 0:
			return pos==0 || pos==1 ? PP : BK;
		case 1:
			return pos==1 || pos==2 ? PP : BK;
		case 2:
			return pos==2 || pos==3 ? PP : BK;
		}
		return BK;
	}
	/**
